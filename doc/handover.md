...
> 最後更新：2026-04-20（Phase 0–3 完成，Phase 4 待執行）
...
### bundle.py 合併順序
`Icons → constants → MetricComponents → PlayTypeCard → TrackingCardRow → App`，依序拼接寫入 `dist/components-bundle.js`。
...
| `WolvesBackfill` | 每日 15:00 | `run_backfill_next.bat` | 從 `backfill_queue.txt` 讀第一行執行，成功則移除該行；queue 空跳過 | `backfill_log.txt` |

**手動執行**：
```powershell
schtasks /run /tn "WolvesTracker"
schtasks /run /tn "WolvesBackfill"
```

**`WolvesBackfill` 已可刪除**（queue 已清空，Phase 1 完成）：
```powershell
schtasks /delete /tn "WolvesBackfill" /f
```
...
```
欄位：`date`, `season`, `seasonType`, `type`, `timestamp`, `stats`, `tracking`

球員以 **playerName** 為 key（每日快照沿用舊結構），含 `stats[]` 與 tracking 同一層。
...
### 寫入前去重
`fetch_data.py` 寫入前會抓最近 14 天最新一筆，深度比對 `stats + tracking`（list of dicts 會 normalize 排序），完全相同則跳過寫入避免堆積無意義文件。
...
### Q: 想新增 Tracking 類型
1. `fetch_data.py` 的 `TRACKING_TYPES` 加新 `PtMeasureType`
2. `fetch_tracking_data()` 加對應 `elif` 處理
3. `components/constants.js` 的 `trackingDefs` 加新項（含 `englishLabel`）
4. `python scripts/bundle.py` 重新打包

### Q: 想追蹤其他球隊
改 `scripts/fetch_data.py` 的 `TEAM_ID` 常數。歷史回補也需要同步調整 `backfill_history.py` 的隊伍 ID。
...
- **不用 GitHub Actions**：datacenter IP 容易被 NBA 封鎖，改用本機 Windows 排程（家用 IP）
